...
def show_result(images_list):
    for i, (title, img) in enumerate(images_list.items()):
...
def main():
    ...
    for image_file in image_files:
        if image_file.endswith(('.png', '.jpg', '.jpeg')):
            ...
            img, size, dimension = read_image(img_path)

    # Read Image
    # img, size, dimension = read_image("../sample/frame_11.png")
        ...
        output_folder = '../res/out_interpolation/'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        # save_image(czos_img, output_folder, 'lanczos_interpolated_image.png')
        save_image(czos_img, output_folder, f'lanczos_interpolated_{image_file}')
